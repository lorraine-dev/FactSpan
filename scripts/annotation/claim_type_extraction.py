...
BATCH_SIZE = 15
CLAIMTYPE_CATEGORIES = "(Opinion|Factual)"
LOG_FILE = "logs/claimtype_annotation_log.txt"
OUTPUT_CSV_PREFIX = "logs/claimtype_extraction_"
...
def get_model_responses(claims_batch: list, base_prompt: str, model: str) -> list:
    ...
    try:
        ...
        verdicts = [re.search(rf"{i}\.\s*{CLAIMTYPE_CATEGORIES}", message_content, re.IGNORECASE | re.DOTALL).group(1).strip() if re.search(rf"{i}\.\s*{CLAIMTYPE_CATEGORIES}", message_content, re.IGNORECASE | re.DOTALL) else 'Reevaluate' for i in range(1, len(claims_batch) + 1)]
        logging.info(f"\n{verdicts}\n")
...
def extract_claimtypes(subset_data: pd.DataFrame, base_prompt: str, result_col: str, model: str) -> pd.DataFrame:
...
def annotate_dataset(data: pd.DataFrame):
    ...
    result_col = 'claim type'
    data['processed'] = False
    data[result_col] = pd.Series(dtype='object')
    base_prompt = """
    Determine the nature of each claim below, classifying it as either 'Factual' or 'Opinion', based on how it is framed in the text. Use these definitions:
    Factual: A claim asserted as a fact.
    Opinion: A claim that expresses personal belief or subjective interpretation.
    Please respond with the classification in English, even if the claim is in a non-English language. Analyze the content and context of each claim to determine its type.
    Claims:
    """
    ...
    while claims_left > 0:
        control_batch_processing(data, base_prompt, result_col, total_batches=1, batch_size=150, process_func=extract_claimtypes)
...
if __name__ == "__main__":
    ...
    try:
        ...
        data.to_csv(f'{args.input_csv.replace(".csv", "_annotated_ct.csv")}', index=False)
        print(f"Annotation completed. Annotated data saved to {args.input_csv.replace('.csv', '_annotated_ct.csv')}")
        logging.info(f"Annotation completed. Annotated data saved to {args.input_csv.replace('.csv', '_annotated_ct.csv')}")
